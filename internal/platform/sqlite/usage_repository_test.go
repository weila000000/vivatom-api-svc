...
func TestUsageReservesCreditsAndEnforcesWorkspaceLimit(t *testing.T) {
	...
	service := usage.NewService(identityService, agent.NewOrchestrator(ai.NewFakeProvider(), generation.NewGuard()), NewUsageRepository(database))
	workspaceID := owner.Workspaces[0].ID

	for index := 0; index < 3; index++ {
		events, runErr := service.Run(ctx, owner.Session.Token, workspaceID, domain.AgentRequest{Action: domain.ActionBuild, ProjectID: "p1", Prompt: "build", Plan: &domain.BuildPlan{}})
		if runErr != nil {
			t.Fatal(runErr)
		}
		for range events {
	...
	_, err = service.Run(ctx, owner.Session.Token, workspaceID, domain.AgentRequest{Action: domain.ActionBuild, ProjectID: "p1", Prompt: "build", Plan: &domain.BuildPlan{}})
